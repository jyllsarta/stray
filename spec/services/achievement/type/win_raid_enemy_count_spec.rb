...
RSpec.describe Achievement::Type::WinRaidEnemyCount, type: :model do
  let!(:user){ create(:user) }
  let!(:user_status){ create(:user_status, user: user) }
  let!(:achievement){ create(:achievement, type: "Achievement::Type::WinRaidEnemyCount", target_id: enemy.id) }
  ...
  describe "#progress_achievement" do
    ...
    before do
      allow(user).to receive_message_chain(:won_enemies, :daily, :count).and_return(won_enemies_count)
    end
    context "正常系" do
