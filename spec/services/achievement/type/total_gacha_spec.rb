...
RSpec.describe Achievement::Type::TotalGacha, type: :model do
  let!(:user){ create(:user) }
  let!(:user_status){ create(:user_status, user: user) }
  let!(:achievement){ create(:achievement, type: "Achievement::Type::TotalGacha") }
  ...
  describe "#progress_achievement" do
    ...
    before do
      allow(user).to receive_message_chain(:gacha_point, :point).and_return(count)
    end
    context "正常系" do
      let(:count){ 1 }
      let(:progress){ 0 }
